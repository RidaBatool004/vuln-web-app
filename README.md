...
| **v2.0.1** | Students who want the reference **plus persistent login sessions** | Everything in v2.0.0 plus **Remember Me**: users can choose to keep their session active across browser restarts using a persistent session cookie (default 30 days, configurable via `REMEMBER_ME_MAX_AGE`). When unchecked, login behaves exactly as before using a session cookie. Supports normal login and both Email OTP and TOTP authentication flows. No database schema change and no new dependency. |

The incremental tags between them (**v0.1.2 – v0.1.7**) each close one additional vulnerability — see the [Bug Fixes](#bug-fixes) table for the version-by-version mapping. The feature-enhancement tags build on top of v1.0.0: **v1.0.1** adds the password strength meter, **v1.0.2** the User Profile Page, **v1.0.3** Continue with Google, **v1.0.4** Email Verification on Signup, **v1.0.5** Account Lockout, **v1.0.6** Email OTP 2FA, **v1.0.7** MFA via Authenticator App (TOTP), **v1.0.8** QR Code Login, and **v2.0.0** CAPTCHA on Login, **v2.0.1** Remember Me on Login.
...
| POST | `/login` | Authenticate user and optionally create a persistent session when "Remember Me" is selected (returns JSON) | No |
...
| **v2.0.1** | 9 | Remember Me on Login | Adds an optional **Remember Me** checkbox on the login page that creates a **persistent session cookie** (default 30 days, configurable via `REMEMBER_ME_MAX_AGE`). The feature integrates with normal login, Email OTP, and TOTP flows while preserving the existing session-based authentication model. No database schema change and no new dependency. | **Done** |
