...
| 12 | Remember Me | Allow users to remain signed in across browser sessions by securely storing a persistent authentication token when they select the Remember Me option during login. The remembered session should expire after a configurable period and be cleared on logout. | DONE (via Codex) |
